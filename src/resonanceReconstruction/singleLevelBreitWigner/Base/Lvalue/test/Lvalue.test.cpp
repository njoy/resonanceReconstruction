...
Base::Lvalue makeLvalue(){
  ...
  auto target2CompoundWeightRatio = awr / (awr + 1.);
  ...
  const auto processResonance = [&]( auto&& chunk ){
    ...
    const auto channelRadius = radius( awr )( std::abs(energy) );
  ...
  return { std::move(resonances), std::nullopt, target2CompoundWeightRatio, l };
}
