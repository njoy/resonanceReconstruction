...
SCENARIO("Single SLBW resonance reconstruction"){
  ...
  SECTION( "third resonance of pu238 of ENDF 6" ){
    ...
    Base::Lvalue::Resonance resonance{ resonanceEnergy,
                                       neutronWidth,
                                       captureWidth,
                                       fissionWidth,
    ...
    auto resonanceXS = evaluate( resonance, resonanceEnergy );
    auto resonanceElastic = std::get<0>(resonanceXS.data);
    auto resonanceCapture = std::get<1>(resonanceXS.data);
    auto resonanceFission = std::get<2>(resonanceXS.data);
    ...
    GIVEN("an evaluation at the right full width half max energy"){
      ...
      auto fwhmXS = evaluate( resonance, energy );
      auto fwhmElastic = std::get<0>(fwhmXS.data);
      auto fwhmCapture = std::get<1>(fwhmXS.data);
      auto fwhmFission = std::get<2>(fwhmXS.data);
      auto fwhmTotal = fwhmElastic
                       + fwhmCapture
                       + fwhmFission;

      THEN("the shape will be correct"){
        REQUIRE( 0.5 * resonanceTotal.value
                 == Approx( fwhmTotal.value ).epsilon(0.005) );
      }
    ...
    GIVEN("an evaluation at the leftt full width half max energy") {
      auto energy = resonanceEnergy - 0.5 * totalWidth;
      auto fwhmXS = evaluate( resonance, energy );
      auto fwhmElastic = std::get<0>(fwhmXS.data);
      auto fwhmCapture = std::get<1>(fwhmXS.data);
