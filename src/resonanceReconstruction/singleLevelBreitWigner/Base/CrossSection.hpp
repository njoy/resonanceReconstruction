struct CrossSection {
  ...
  CrossSection( const Pack< T1, T2, T3 >& pack ) :
    elastic( std::get<0>(pack) ),
  ...
  CrossSection operator*( const double scalar ) const {
    return { this->elastic * scalar,
             this->capture * scalar,
             this->fission * scalar }; 
  }

  friend std::ostream& operator<<( std::ostream& os, const CrossSection& xs ){
    os << '[' << xs.elastic << xs.capture << xs.fission << ']';
    return os;
  }
};
