...
SCENARIO( "SpinGroup" ) {
  ...
  GIVEN( "valid data for a SpinGroup" ) {
    ...
    ParticlePair incident( neutron, fe54, 0.0 * electronVolt );
    Channel< Neutron > elastic( "2", incident, { 0, 0.5, 0.5, +1 },
    ...
    THEN( "a SpinGroup can be constructed using the actual indices" ) {
      SpinGroup< Sammy > group( { 0 }, { elastic }, std::move( single ) );
    ...
    THEN( "a SpinGroup can be constructed using the incident particle pair" ) {
      SpinGroup< Sammy > group( incident, { elastic }, std::move( single ) );
