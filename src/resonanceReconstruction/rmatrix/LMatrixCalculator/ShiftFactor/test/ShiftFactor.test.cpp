...
SCENARIO( "LMatrixCalculator< ShiftFactor >" ) {
  ...
  GIVEN( "valid data for channels" ) {
    ...
    THEN( "a calculator can be constructed" ) {
      ...
      LMatrixCalculator< ShiftFactor > calculator( channels.size() );
    ...
    THEN( "L = S - B + iP can be calculated" ) {
      ...
      CHECK( std::complex< double >( 0., 1. ) == matrix.diagonal()[0] );
      CHECK( std::complex< double >( 0., 2. ) == matrix.diagonal()[1] );
      CHECK( std::complex< double >( 0., 3. ) == matrix.diagonal()[2] );
    }
  } // GIVEN
} // SCENARIO
