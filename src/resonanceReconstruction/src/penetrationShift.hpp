...
public:
   ...
   std::array<double,2> operator()(const double channelRatio) const
   {
      ...
      if (L == 0) {
         ...
         r = 225 + rho2*(45 + rho2*(6 + rho2));
         a = rho4*rho2;
         b = 675 + rho2*(90 + rho2*6);
      } else if (L == 4) {
         r = 11025 + rho2*(1575 + rho2*(135 + rho2*(10 + rho2)));
         a = rho4*rho4;
         b = 44100 + rho2*(4725 + rho2*(270 + rho2*10));
      } else {
         throw std::exception();
      }

      return {{ (channelRatio > 0)*channelRatio*a/r, -b/r }};
   }
};
